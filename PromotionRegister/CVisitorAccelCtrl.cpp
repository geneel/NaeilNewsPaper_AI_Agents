...
IMPLEMENT_DYNAMIC(CVisitorAccelCtrl, CStatic)

BEGIN_MESSAGE_MAP(CVisitorAccelCtrl, CStatic)
    ON_WM_PAINT()
END_MESSAGE_MAP()

CVisitorAccelCtrl::CVisitorAccelCtrl() {}
CVisitorAccelCtrl::~CVisitorAccelCtrl() {}

void CVisitorAccelCtrl::SetDataPtr(const double* pData, int dataCount)
{
    m_pData = pData;
...
void CVisitorAccelCtrl::OnPaint()
{
    CPaintDC dc(this);
    CRect rc;
    GetClientRect(&rc);
...
void CVisitorAccelCtrl::DrawGraph(CDC* pDC, const CRect& rc)
{
    // 상대좌표 변환 람다
