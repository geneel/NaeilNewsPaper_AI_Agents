...
IMPLEMENT_DYNAMIC(CArtWriterCtrl, CStatic)

BEGIN_MESSAGE_MAP(CArtWriterCtrl, CStatic)
    ON_WM_PAINT()
END_MESSAGE_MAP()

CArtWriterCtrl::CArtWriterCtrl() {}
CArtWriterCtrl::~CArtWriterCtrl() {}

void CArtWriterCtrl::SetDataPtr(const double* pData, int dataCount)
{
    m_pData = pData;
...
void CArtWriterCtrl::DrawGraph(CDC* pDC, const CRect& rc)
{
    // 상대좌표 변환 람다
