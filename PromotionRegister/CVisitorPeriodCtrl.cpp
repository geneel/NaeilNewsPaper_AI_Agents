...
IMPLEMENT_DYNAMIC(CVisitorPeriodCtrl, CStatic)

BEGIN_MESSAGE_MAP(CVisitorPeriodCtrl, CStatic)
    ON_WM_PAINT()
END_MESSAGE_MAP()

CVisitorPeriodCtrl::CVisitorPeriodCtrl() {}
CVisitorPeriodCtrl::~CVisitorPeriodCtrl() {}

void CVisitorPeriodCtrl::SetDataPtr(const double* pData, int dataCount)
{
    m_pData = pData;
...
void CVisitorPeriodCtrl::OnPaint()
{
    CPaintDC dc(this);
    CRect rc;
    GetClientRect(&rc);
...
void CVisitorPeriodCtrl::DrawGraph(CDC* pDC, const CRect& rc)
{
    // 상대좌표 변환 람다
