...
IMPLEMENT_DYNAMIC(CArtEmbeddingCtrl, CStatic)

BEGIN_MESSAGE_MAP(CArtEmbeddingCtrl, CStatic)
    ON_WM_PAINT()
END_MESSAGE_MAP()

CArtEmbeddingCtrl::CArtEmbeddingCtrl() {}
CArtEmbeddingCtrl::~CArtEmbeddingCtrl() {}

void CArtEmbeddingCtrl::SetDataPtr(const double* pData, int dataCount)
{
    m_pData = pData;
...
void CArtEmbeddingCtrl::OnPaint()
{
    CPaintDC dc(this);
    CRect rc;
    GetClientRect(&rc);
...
void CArtEmbeddingCtrl::DrawGraph(CDC* pDC, const CRect& rc)
{
    // 상대좌표 변환 람다
